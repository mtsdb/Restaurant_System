  ...
  const load = async () => {
    ...
    try {
      const [{ data: list }, { data: dash }] = await Promise.all([
        baristaApi.listBaristaItems(statusFilter ? { status: statusFilter } : undefined),
        baristaApi.baristaDashboard(statusFilter ? { status: statusFilter } : undefined),
      ])
      setItems(list)
      setDashboard(dash)
    } catch (e: any) {
      setError(e?.response?.data?.detail || 'Failed to load barista data')
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Barista</h2>
      ...
      {dashboard && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="card"><div className="text-sm text-gray-500">Waiting</div><div className="text-2xl font-semibold">{dashboard.drink?.waiting ?? 0}</div></div>
          <div className="card"><div className="text-sm text-gray-500">In Progress</div><div className="text-2xl font-semibold">{dashboard.drink?.in_progress ?? 0}</div></div>
          <div className="card"><div className="text-sm text-gray-500">Ready</div><div className="text-2xl font-semibold">{dashboard.drink?.ready ?? 0}</div></div>
        </div>
      )}
